        ...
        <c:choose>
            <c:when test="${segnalazione.statoSegnalazione.toString().equalsIgnoreCase('Attiva')}">
                <tr class="clickable clickable-row">
                    ...
                    <td onclick="window.location.href ='${pageContext.request.contextPath}/campagna/campagna?idCampagna=${segnalazione.campagna.idCampagna}'">${segnalazione.campagna.idCampagna}</td>
                    ...
                    <td>
                        <form action="${pageContext.request.contextPath}/segnalazioni/risolvi" method="post">
                            <input type="hidden" name="idCampagna" value="${segnalazione.campagna.idCampagna}">
            ...
            <c:otherwise>
                <tr class="clickable clickable-row"
                    onclick="window.location.href ='${pageContext.request.contextPath}/campagna/campagna?idCampagna=${segnalazione.campagna.idCampagna}'">
                    <th scope="row">${segnalazione.idSegnalazione}</th>
                    <td>${segnalazione.campagna.idCampagna}</td>
                    <td>${segnalazione.descrizione}</td>
                    <td>${segnalazione.statoSegnalazione}</td>
                </tr>
            </c:otherwise>
